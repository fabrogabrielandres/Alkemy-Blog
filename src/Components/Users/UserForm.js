    ...
    <div>
        ...
        <Formik
            ...
            onSubmit={ (values) => {
                ...
                if(user.name) {
                    //Also, the method will be different 
                    //if there is somethin into "user.name"
                    //Doing a "put" method to edit or a "post" to create
                    console.log(values)
                    axios.put(`http://ongapi.alkemy.org/api/users/${user.id}#t53`, values)
                    .then(response => console.log(response))
                    .catch(err => console.log(err))
            
                } else {
                    axios.post('http://ongapi.alkemy.org/api/users#t53/', values)
                    .then(response => console.log(response))
                    .catch(err => console.log(err))  
                }
            }}
